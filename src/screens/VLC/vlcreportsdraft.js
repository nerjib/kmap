...
export default class VLCreportsDraft extends React.Component{
    constructor(props){
        super(props)
        this.state={
            tableview:'none',
            tableHead: ['Location','functionality','Image'],
            norecord: '',
            receivedData:'',
            functionality:'',
            problem:'',
...
Object.keys(this.state.receivedData).map((e,i)=>data2.push([this.state.receivedData[e].community+' '+this.state.receivedData[e].ward,this.state.receivedData[e].functionality,
...
.then(res=>{
 ...
 //alert(JSON.stringify(kkkh))
 let newData=JSON.stringify(oldData)
 AsyncStorage.setItem('VLCdraft',newData);
